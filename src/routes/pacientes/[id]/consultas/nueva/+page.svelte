<script lang="ts">
  ...
  function validate(): string | null {
    ...
    if (Number.isNaN(ct) || ct < 0) return 'El costo total debe ser un número válido mayor o igual a 0.';
    return null;
  }
  ...
  async function handleSubmit() {
    ...
    submitting = true;
    try {
      // Crear consulta (si fecha_consulta está vacía, se usa timestamp actual)
      ...
      // Crear tratamiento asociado
      const tratamiento = createTratamiento({
...
<section class="nueva-consulta">
  ...
  {:else if paciente}
    <Card title="Nueva consulta" subtitle={`Paciente: ${paciente.nombre_completo}`} hoverable outlined>
      <div class="grid-2">
        <div class="info-item">
