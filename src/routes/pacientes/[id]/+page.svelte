<script lang="ts">
  ...
  function reloadData() {
    ...
    error = null;
    try {
      seedMockData(); // asegura datos en entorno mock
      ...
      consultas = listConsultasByPaciente(paciente.id);

      // Obtener tratamientos de todas las consultas y calcular saldo
      const tratamientosRaw: Tratamiento[] = consultas.flatMap((c) => listTratamientosByConsulta(c.id));
      tratamientos = tratamientosRaw.map(computeTratamientoSaldo);

      const resumen = getPacienteResumen(paciente.id);
      totalSaldoPaciente = resumen?.totalSaldoPaciente ?? 0;

      loading = false;
    } catch (e: any) {
      loading = false;
      error = e?.message || 'Error cargando datos del paciente';
    }
  }
  ...
  function togglePagos(tratamientoId: number) {
    openPagos[tratamientoId] = !openPagos[tratamientoId];
  }
  ...
  {:else if error}
    <Card padded outlined>
      ...
      <div class="row-actions">
        <Button variant="outline" on:click={reloadData}>Reintentar</Button>
      </div>
    </Card>
  {:else if paciente}
    <!-- Resumen del paciente -->
    <Card title="Paciente" subtitle="Datos generales" hoverable outlined>
      <div class="grid-2">
        <div class="info-item">
          <div class="label">Nombre</div>
          <div class="value">{paciente.nombre_completo}</div>
        </div>
        <div class="info-item">
          <div class="label">Identificación</div>
          <div class="value">{paciente.numero_identificacion}</div>
        </div>
        <div class="info-item">
        ...
        <div class="info-item">
          ...
          <div class="value">{formatDate(paciente.fecha_creacion)}</div>
        </div>
        <div class="info-item">
          <div class="label">Estado</div>
          <div class="value">
            <Badge variant={paciente.activo ? 'success' : 'neutral'} pill>
      ...
      <div class="saldo-total">
        ...
        <div class="value">
          <Badge variant={estadoSaldoBadgeVariant(totalSaldoPaciente)} pill>
      ...
      <div class="row-actions">
        <Button variant="primary" on:click={gotoNuevaConsulta}>Nueva consulta</Button>
        <Button variant="outline" on:click={() => (window.location.href = '/pacientes')}>Volver a listado</Button>
      </div>
    </Card>

    <!-- Historial de consultas -->
    <Card title="Historial de consultas" subtitle="Ordenadas por fecha" hoverable>
            ...
            {:else}
              {#each consultas as c}
                <tr>
                  ...
                  <td>
                    <div class="row-actions">
                      <Button variant="secondary" size="sm" on:click={() => (window.location.href = `/pacientes/${paciente?.id}/consultas/${c.id}`)}>
                        Ver
                      </Button>
                      <Button variant="outline" size="sm" on:click={() => (window.location.href = `/pacientes/${paciente?.id}/consultas/${c.id}/editar`)}>
                        Editar
                      </Button>
                    </div>
                  </td>
    ...
    <Card title="Tratamientos" subtitle="Detalle y saldo por tratamiento" hoverable>
      <div class="tabla-wrapper">
        <table class="table">
          <thead>
            <tr>
            ...
            {:else}
              {#each tratamientos as t}
                <tr>
                  ...
                  <td>
                    <Badge variant={estadoSaldoBadgeVariant(t.saldoPendiente)} pill>
                      {formatMoney(t.saldoPendiente)}
                    </Badge>
                  </td>
                  <td>
                    <div class="row-actions">
                      <Button variant="secondary" size="sm" on:click={() => togglePagos(t.id)}>
                        {openPagos[t.id] ? 'Ocultar pagos' : 'Ver pagos'}
                      </Button>
                      <Button variant="outline" size="sm" on:click={() => gotoRegistrarPago(t.id)}>
                        Registrar pago
                      </Button>
                      <Button variant="outline" size="sm" on:click={() => (window.location.href = `/laboratorios/nuevo?tratamiento=${t.id}`)}>
                        Trabajo laboratorio
                      </Button>
                    </div>
                  </td>
