<script lang="ts">
	...
	import {
		...
		marcarEntregado,
		type Laboratorio,
		type TrabajoLaboratorio as TrabajoSrv
	...
	// Mapeo local de estados a variantes de badge
	function estadoVariant(
		estado: Trabajo['estado'] | string
	...
	// Catálogo de laboratorios (servicio)
	let laboratoriosCatalogo: string[] = [];
	// Mapa id -> nombre para lookup rápido al mapear trabajos
	let laboratoriosCatalogoMap: Map<number, string> = new Map();
	...
	$: filteredTrabajos = trabajos
		.filter((t) => {
			...
			const byPendiente = mostrarSoloPendientes
				? !['ENTREGADO', 'CANCELADO'].includes(t.estado)
				: true;
	...
	async function marcarEntregadoClick(id: number) {
		// Llamar al API para marcar entregado y refrescar la lista desde el servidor
		try {
			await marcarEntregado(id);
			// volver a consultar trabajos
			const trabajosSrv = await listTrabajosLaboratorio();
			trabajos = trabajosSrv.map((t) => ({
				id: t.id,
				paciente: t.consulta_id ? `Consulta #${t.consulta_id}` : '—',
				laboratorio: laboratoriosCatalogoMap.get(t.laboratorio_id) ?? '—',
				estado: t.estado as unknown as Trabajo['estado'],
				fecha_solicitud: t.fecha_recepcion ?? '—',
				fecha_estimada_entrega: t.fecha_entrega_paciente ?? '—',
				costo_laboratorio: null
			}));
		} catch (err) {
			// mostrar en consola; la UI puede mejorarse para notificar al usuario
			// eslint-disable-next-line no-console
			console.error('Error marcando trabajo como entregado', err);
		}
	}
	onMount(async () => {
		// Cargar catálogo y trabajos desde el API
		try {
			const labs = await listLaboratorios();
			laboratoriosCatalogo = labs.map((l: Laboratorio) => l.nombre);
			const labMap = new Map<number, string>();
			for (const l of labs) {
				if (typeof l.id === 'number') labMap.set(l.id, l.nombre);
			}
			laboratoriosCatalogoMap = labMap;

			const trabajosSrv = await listTrabajosLaboratorio();
			trabajos = trabajosSrv.map((t) => ({
				id: t.id,
				paciente: t.consulta_id ? `Consulta #${t.consulta_id}` : '—',
				laboratorio: labMap.get(t.laboratorio_id) ?? '—',
				estado: t.estado as unknown as Trabajo['estado'],
				fecha_solicitud: t.fecha_recepcion ?? '—',
				fecha_estimada_entrega: t.fecha_entrega_paciente ?? '—',
				costo_laboratorio: null
			...
			// eslint-disable-next-line no-console
			console.error('Error cargando laboratorios o trabajos desde API', err);
		}
	});
</script>
							...
							<tr>
								...
								<td>
									<div class="row-actions">
										<Button variant="secondary" size="sm" on:click={() => verTrabajo(t.id)}
											>Ver</Button
										>
										{#if t.estado !== 'ENTREGADO' && t.estado !== 'CANCELADO'}
											<Button
												variant="outline"
												size="sm"
												on:click={() => marcarEntregadoClick(t.id)}>Marcar entregado</Button
											>
										{/if}
									</div>
								</td>
